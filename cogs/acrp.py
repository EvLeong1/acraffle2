    ...
    async def raffleplus(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for x in range(len(acrpRarities)):
            if result[0] == acrpRarities[x]:
                ...
                if newuser == True:
                    em = discord.Embed(title = f"ACraffleplus - {member.name} got {Char['name'].capitalize()}",color = getColor(Char['rarity']))
    ...
    async def acrp(self,interaction: discord.Interaction):
        ...
        try:
            ...
            if currenttime < endcooldown:
                ...
                em = discord.Embed(title = f"ACraffleplus Cooldown - {member.name}",description = '**/acrp** is on cooldown for you.',color = discord.Color.teal())
